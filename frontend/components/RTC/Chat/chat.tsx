  ...
  return (
    <div className="flex flex-col h-full bg-background rounded-l-2xl overflow-hidden">
      <div ref={scrollerRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
            ...
            <div key={idx} className={`flex ${isSystem ? "justify-center" : mine ? "justify-end" : "justify-start"}`}>
              ...
              >
                {isSystem ? (
                  ...
                  <>
                    {!mine && <div className="text-[10px] text-muted-foreground mb-1">Peer</div>}
                    <div>{m.text}</div>
                  </>
                )}
              </div>
        ...
        {peerTyping && <div className="text-xs text-muted-foreground italic">{peerTyping}</div>}
      </div>

      <div className="p-3 divider-match">
        <div className="flex items-center gap-2">
          <input
            className="flex-1 bg-background input-border-match rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:ring-2 focus:ring-ring"
          ...
          <button
            ...
            className="cursor-pointer h-10 px-4 rounded-xl bg-muted hover:bg-muted/80 disabled:opacity-50 text-foreground text-sm font-medium transition-colors shadow-sm input-border-match"
          >
            Send
          </button>
        </div>
